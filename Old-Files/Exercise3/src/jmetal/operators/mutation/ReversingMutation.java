...
/**
 * This class implements a Reversing Mutation operator.
 * In case of reversing mutation applied for binary encoded chromosome, random position is chosen and 
 * the bits next to that position are reversed and child chromosome is produced
 */
public class ReversingMutation extends Mutation {
	...
	/**
	 * Constructor
	 * Creates a new instance of the ReversingMutation  operator
	 */
	public ReversingMutation(HashMap<String, Object> parameters) {
		super(parameters) ;
	...
	public void doMutation(double probability, Solution solution) throws JMException {
		try {
			if ((solution.getType().getClass() == BinarySolutionType.class) ||
				...
				while(randomnumber < probability){
					...
					for (int i = 0; i < solution.getDecisionVariables().length; i++) {
						...
						// move one bit if on boundary
						if (index == 0)index = index + 1;
						if (index == ((Binary)solution.getDecisionVariables()[i]).getNumberOfBits()) index = index - 1;
						// Flip the two neighbours
						((Binary) solution.getDecisionVariables()[i]).bits_.flip(index+1);
						((Binary) solution.getDecisionVariables()[i]).bits_.flip(index-1);
					}
					// do another round according to the probability
			...
			Configuration.logger_.severe("ReversingMutation.doMutation: " +
	...
	public Object execute(Object object) throws JMException {
		...
		if (!VALID_TYPES.contains(solution.getType().getClass())) {
			Configuration.logger_.severe("ReversingMutation.execute: the solution " +
