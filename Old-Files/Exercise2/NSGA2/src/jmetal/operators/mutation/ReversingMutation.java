			...
			if ((solution.getType().getClass() == BinarySolutionType.class) ||
				...
				while(randomnumber < probability){
					...
					for (int i = 0; i < solution.getDecisionVariables().length; i++) {
						...
						if ( ((Binary)solution.getDecisionVariables()[i]).getNumberOfBits() == 0){
							System.out.println("WrongLength: "+((Binary)solution.getDecisionVariables()[i]).getNumberOfBits());
//							continue;
						}
							System.out.println("Index: "+index);
							System.out.println("Length: "+((Binary)solution.getDecisionVariables()[i]).getNumberOfBits());
