 ...
 *
 */
public class newPWT extends Problem {
	 ...
	 * @throws IOException
	 */
	public newPWT(String solutionType, String file, String tourfile) throws IOException {
		//store the file name
		this.filename = file;
	...
	public void evaluate(Solution solution) {
		...
		int ItemsPerCity = this.numberOfItems_ / (tour.length - 2);
		...
		for (int i=1; i<tour.length-1; i++) {
			...
			for(int j = 0; j < ItemsPerCity; j++){
				int indexOfPackingPlan = (i-1)*ItemsPerCity+j;
				int indexOfItem = currentCity+j*(this.numberOfCities_-1);
				//if the items is in the current city
//				if(items[j][3] == currentCity){
					//if the items is picked
					if(packingPlan[indexOfPackingPlan] == 1){
						finalprofit = finalprofit + items[indexOfItem][1];
						totalWeight = totalWeight + items[indexOfItem][2];
					}
				}
