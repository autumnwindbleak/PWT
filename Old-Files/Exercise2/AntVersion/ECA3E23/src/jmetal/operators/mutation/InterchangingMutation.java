...
public class InterchangingMutation extends Mutation {
	...
	/**
	 * Constructor
	 * Creates a new instance of the InterchangingMutation operator
	 */
	public InterchangingMutation(HashMap<String, Object> parameters) {
		super(parameters) ;
	...
	public void doMutation(double probability, Solution solution) throws JMException {
		try {
			if ((solution.getType().getClass() == BinarySolutionType.class) ||
				...
				while(randomnumber < probability){
					...
					for (int i = 0; i < solution.getDecisionVariables().length; i++) {
						...
						int index1 = (int) (PseudoRandom.randDouble() * ((Binary)solution.getDecisionVariables()[i]).getNumberOfBits());
						int index2 = (int) (PseudoRandom.randDouble() * ((Binary)solution.getDecisionVariables()[i]).getNumberOfBits());
						// swap the bits of the two positions
						boolean swap = ((Binary) solution.getDecisionVariables()[i]).bits_.get(index1);
						((Binary) solution.getDecisionVariables()[i]).bits_.set(index1, ((Binary) solution.getDecisionVariables()[i]).bits_.get(index2));
						((Binary) solution.getDecisionVariables()[i]).bits_.set(index2, swap);

					}
					// Do one more time accorrding to the mutation probability
			...
			Configuration.logger_.severe("InterchangingMutation.doMutation: " +
	...
	public Object execute(Object object) throws JMException {
		...
		if (!VALID_TYPES.contains(solution.getType().getClass())) {
			Configuration.logger_.severe("InterchangingMutation.execute: the solution " +
